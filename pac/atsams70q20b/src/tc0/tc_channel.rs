...
impl TC_CHANNEL {
    ...
    #[inline(always)]
    pub fn cmr_waveform_mode(&self) -> &CMR_WAVEFORM_MODE {
        unsafe { &*(((self as *const Self) as *const u8).add(4usize) as *const CMR_WAVEFORM_MODE) }
    }
    #[doc = "0x04 - Channel Mode Register (channel = 0)"]
    #[inline(always)]
    pub fn cmr_capture_mode(&self) -> &CMR_CAPTURE_MODE {
        unsafe { &*(((self as *const Self) as *const u8).add(4usize) as *const CMR_CAPTURE_MODE) }
    }
}
